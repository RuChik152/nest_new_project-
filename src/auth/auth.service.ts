...
export class AuthService {
  constructor( @InjectModel(AdminUser.name) private authModel: Model<AdminAuthDocument>) {}
  ...
  async find(findAdminUserDto: AdminUserDto) {
    const findUser = await this.authModel.findOne({email: findAdminUserDto.email})
    return findUser;
  }
